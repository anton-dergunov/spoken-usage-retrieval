...
def test_acquisition_is_round_robin_limited_and_resumable(tmp_path):
    config = {
        "channels": [
            {"id": "one", "name": "One", "url": "https://one.example/videos", "varieties": ["Spain"]},
            {"id": "two", "name": "Two", "url": "https://two.example/videos", "varieties": ["Mexico"]},
        ]
    }
    ...
    def runner(arguments):
        if "--flat-playlist" in arguments:
            ...
            return json.dumps({"entries": [{
                "id": f"video-{channel}", "url": f"https://youtube.test/video-{channel}",
                "title": channel, "duration": 120, "live_status": "not_live",
            }]})
        ...
        output.write_text(json.dumps({"events": [{
            "tStartMs": 0, "dDurationMs": 1000, "segs": [{"utf8": "Una frase completa."}]
        }]}))
        return ""
